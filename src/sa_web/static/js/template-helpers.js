  ...
  S.TemplateHelpers = {
    ...
    insertInputTypeFlags: function(configItems) {
      _.each(configItems, function(item, index) {
        item.is_input = (!item.type || (item.type !== 'textarea' && item.type !== 'select' && item.type !== 'file' && item.type !== 'multi-checkbox' && item.type !== 'multi-radio' ));
        ...
        item.is_multi_radio = (item.type === 'multi-radio');
      });
    },
